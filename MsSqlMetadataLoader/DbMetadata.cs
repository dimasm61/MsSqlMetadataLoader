...
namespace MsSqlMetadataLoader
{
    public class DbMetadata
    {
        ...
        public static string ConnStr = @"Server=srv-4\EX2014_KP;Database=VTSDB3;User Id=user_sql;Password=user_sql;";

        /// <summary>
        /// <example> var db1 = DbMetadata.Load().FilterByScheme("+", "nav", "usr").FilterByTable("+","nav.Pass","usr.Watch"); </example>
        /// </summary>
        /// <returns></returns>
        public static DbMetadata Load(double minTimeOut = 1)
        {
            var sw = Stopwatch.StartNew();

            var result = new DbMetadata();
            ...
            using (var conn = new SqlConnection(ConnStr))
            {
                ...
                // загружаем все данные о таблицах
